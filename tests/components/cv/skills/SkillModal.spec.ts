...
import SkillModal from "../../../../src/components/cv/skills/SkillModal.vue";
...
describe("SkillModal.vue", () => {
  ...
  it("emits onCreateCvSkill and closeModal when confirm is clicked after selecting skill and mastery", async () => {
    const wrapper = mount(SkillModal, {
      props: baseProps,
      global: { plugins: [vuetify, i18n] },
      attachTo: document.body,
    ...
    await nextTick();
    expect(wrapper.emitted("onCreateCvSkill")).toBeTruthy();
    expect(wrapper.emitted("closeModal")).toBeTruthy();
  });
});
